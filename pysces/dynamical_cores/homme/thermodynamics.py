...
def eval_r_hat_sq_avg(r_hat_i):
  ...
  Parameters
  ----------
  r_hat_i : Array[tuple[elem_idx, gll_idx, gll_idx, lev_idx+1], Float]
      Radial scaling factor ``r_hat = r / a`` at vertical interfaces, from
      :func:`phi_to_r_hat`.

  Returns
  -------
  r_hat_sq : Array[tuple[elem_idx, gll_idx, gll_idx, lev_idx], Float]
...
def eval_mu(state,
  ...
  phi_i : Array[tuple[elem_idx, gll_idx, gll_idx, lev_idx+1], Float]
  ...
  d_nh_pressure_d_mass : Array[tuple[elem_idx, gll_idx, gll_idx, lev_idx+1], Float]

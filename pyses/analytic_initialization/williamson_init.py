...
def init_williamson_steady_state_config(model_config):
  """
...
def eval_williamson_tc2_u(lat,
  ...
  return wind[:, :, :, jnp.newaxis, :] * jnp.ones_like(v_grid["hybrid_a_m"])[jnp.newaxis, jnp.newaxis, jnp.newaxis, :, jnp.newaxis]
...
def init_williamson_steady_state_state(h_grid,
                                       v_grid,
                                       model_config,
                                       test_config,
                                       dims,
  ...
  model_state : model state struct
      Fully initialised model state struct for the geostrophic flow tc
  """
  assert len(v_grid["hybrid_a_i"]) == 2, "Test case does not support multi-layer SW yet."

  def z_surf_total_height_func(lat,
                               lon):
    return (eval_williamson_tc2_hs(lat, lon, test_config),
            eval_williamson_tc2_h(lat, lon, test_config))

  def u_func(lat, lon, v_grid):
    return eval_williamson_tc2_u(lat, lon, v_grid, test_config)[:, :, :, :, 0]

  def v_func(lat, lon, v_grid):
    return eval_williamson_tc2_u(lat, lon, v_grid, test_config)[:, :, :, :, 1]

  model_state = init_model_shallow_water(z_surf_total_height_func,
                                         u_func,
                                         v_func,
                                         h_grid,
                                         v_grid,

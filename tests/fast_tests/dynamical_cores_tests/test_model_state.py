...
@pytest.mark.parametrize("model",
                         [models.homme_hydrostatic, models.cam_se,
...
def test_tracer_sums(model):
  ...
  model_config = init_physics_config(model)
  test_config = init_baroclinic_wave_config(model_config=model_config)
  model_state_1 = cached_baroclinic_state(nx, npt, model,
                                           mountain=False,
                                           moist=True,
                                           eps=1e-3)
  model_state_2 = cached_baroclinic_state(nx, npt, model,
                                           mountain=False,
                                           moist=True,
                                           eps=1e-3)
  val_1 = 1.0
...
@pytest.mark.parametrize("model", list(dynamical_cores), ids=lambda m: m.name)

...
@pytest.mark.parametrize("nx, npt", [(nx, npt) for nx in [6, 7] for npt in test_npts])
def test_gen_metric(nx, npt):
...
@pytest.mark.parametrize("nx, npt", [(nx, npt) for nx in [14, 15] for npt in test_npts])
def test_gen_mass_mat(nx, npt):
...
def test_new_grid_tmp(nx, npt):
  # Inner ``for _ in range(3)`` retained so the three random draws stay
  # within a single test (their per-trial RNG state is shared via
  # ``np.random``).
  np.random.seed(0)
  for _ in range(3):
